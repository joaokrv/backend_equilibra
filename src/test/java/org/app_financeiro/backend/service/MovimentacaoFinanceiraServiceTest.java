...
class MovimentacaoFinanceiraServiceTest {
    ...
    @BeforeEach
    void setUp() {
        // @InjectMocks usa injeção por construtor; entityManager (@PersistenceContext, field
        // injection) não entra por aí. Injeta o mock manualmente.
        ReflectionTestUtils.setField(movimentacaoFinanceiraService, "entityManager", entityManager);
    ...
    void deveDesfazerEfeitoFinanceiroCartaoDespesa() {
        ...
        // Serialização pelo agregado: deve travar o cartão e reler a fatura fresca antes de mutar.
        verify(cartaoService).obterCartaoComBloqueioExclusivo(20L, 1L);
        verify(entityManager).refresh(faturaPadrao);
